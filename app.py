...
def handle_question_submission(qid):
    ...
    if len(responses) != qid:
        # Trying to submit questions out of order.
        flash(f"Invalid question id: {qid}.")
        return redirect(f"/questions/{len(responses)}")

    # Handle the form submission for checkboxes
    choices = request.form.getlist('answer')  # Use getlist to get multiple values

    # Check if no checkboxes are selected
    if not choices:
        flash("Please select at least one answer.")
        return redirect(request.url)  # Redirect back to the current question

    text = request.form.get("text", "")

    # Add this response to the list in the session
    responses.append({"choices": choices, "text": text})
    session[RESPONSES_KEY] = responses
